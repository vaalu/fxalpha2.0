...
from modules.props.ConfigProps import AppLogger, aliceAnt
...
class AliceInstruments():
	...
	def calculate_date_early_by_2_days(self, curr_date, instr, symbol):
		...
		logger.info(instr_symbol)

	def fetch_commodities(self):
		logger.info('Fetching required commodities')
		selected_commodities = aliceAnt['COMMODITIES_MCX']
		...
		for commodity in selected_commodities:
			...
			if instr == None:
				...
				instr = self.alice.get_instrument_by_symbol(exchange='MCX', symbol=symbol_id)
			processed_sym = self.calculate_date_early_by_2_days(today, instr, symbol_id)
