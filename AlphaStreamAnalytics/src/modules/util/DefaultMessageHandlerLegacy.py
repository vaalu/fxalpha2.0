...
class DefaultMessageHandlerLegacy():
	...
	def __defaultHandler(self, message={}):
		...
		hset_key = '%s:%s:%s'%(source_val["instrument_token"], source_val["exchange_timestamp"], curr_ts)
		...
		logger.info('%s : %s : %s'%(hset_key, source_val["exchange_timestamp"], source_val["last_traded_price"]))
