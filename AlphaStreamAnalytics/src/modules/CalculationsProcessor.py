...
class CalculationsProcessor():
	...
	def process_calculations(self):
		...
		instrument = equities[0]
		while process_init_1 < (start_time + (25*duration)):
			keys_time = [('%s:1M:%s'%(instrument["token"],int(process_init_1 - (duration*index))) if (process_init_1 - (duration*index)) > start_time else '%s:1M:%i'%(instrument["token"],int(start_time))) for index in range(0, 20)]
			# logger.info('Series to be calculated: %s'%str(seq))
